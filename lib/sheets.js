...
async function callOnce(url, options) {
  ...
  // script.googleusercontent.com execution URL 404s on Node's fetch, which
  // sends no User-Agent by default — curl (which defaults to "curl/...")
  // and every browser work fine. An explicit User-Agent fixes most of it,
  // but that hop is also just intermittently flaky on Google's side
  // (observed ~1 in 5 requests still 404 even with a User-Agent set) — see
  // the retry loop in call() below. Either way this response is Google's
  // own HTML error page, not JSON from our script.
